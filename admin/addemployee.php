...
if(isset($_POST['add'])) {
    ...
    if ($image_type == 'image/png' || $image_type == 'image/jpeg' || $image_type == 'image/jpg') {
      ...
      (EmpId,FirstName,LastName,EmailId,Password,Gender,Dob,Department,Address,City,Country,Phonenumber,emp_image,Status) 
       VALUES(:empid,:fname,:lname,:email,:password,:gender,:dob,:department,:address,:city,:country,:mobileno,:emp_image,:status)";
...
<label for="empcode">Employee's Code(Must be unique)</label>
...
<input id="phone" name="mobileno" type="tel" maxlength="10" autocomplete="off" required>
    ...
    <div class="input-field col s12">
        ...
        <br />
        <br />
        <input id="emp_image" name="emp_image" type="file" accept=".png,.jpeg,.jpg"  required>

    </div>
                                                        
<div class="input-field col s12">
<button type="submit" name="add" onclick="return valid();" id="add" class="waves-effect waves-light btn  m-b-xs">ADD</button>
